...
const htsService = require('./htsService')
...
async function getFileInfo(fileId){
  //Create the query
    ...
    const getInfo = await query.execute(const client = hederaClientLocal(process.env.TREASURY_ACCOUNT_ID, process.env.TREASURY_PRIVATE_KEY););
...
async function fileCreate(fileData) {
  ...
  const client = htsService.hederaClientLocal(process.env.TREASURY_ACCOUNT_ID, process.env.TREASURY_PRIVATE_KEY);
  let fileId = "";
