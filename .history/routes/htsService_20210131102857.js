...
//create file
router.route('/create-nfToken').post(async (req, res) => {
...
//create file
router.route('/createTokenWithFileId').post(async (req, res) => {
    const privateKey = await PrivateKey.generate();


    // get parameters from req object
    const fileId = req.query.fileid
    const tokenName = req.query.tokename
    const isKyc = req.query.iskyc
   ...
   // const privateKey = await PrivateKey.generate();
    ...
    if (tokenName !== "") {
        ...
        const token = {
            name: tokenName,
            symbol: fileId !== "" ? "hedera://" + fileId : undefined ,
            memo: fileId !== "" ? "hedera://" + fileId : undefined ,
            decimals: 0,
            initialSupply: 1,
            ...
            wipeKey: undefined,
            supplyKey:  undefined,
            autoRenewAccount: ownerAccountId,
            treasury: ownerAccountId,
            deleted: false,
            key: privateKey.toString()
        };
        const newToken = await tokenCreateModule.tokenCreate(token);
...
router.route('/buy').post(async (req, res) => {
    try {
       // console.log(req);
        ...
        //======
        tx.addTokenTransfer(req.body.token_id, process.env.TREASURY_ACCOUNT_ID, -req.body.amount);
